module Games
  class CreateUnitsImport < ActiveInteraction::Base
    object :game
    ...
    def create_available_units
      %i[Footman Knight Ship SiegeEngine].each do |unit|
    ...
    def create_remaining_units(unit_collection, total_units)
      houses.each do |house|
        remaining = total_units - house.send(unit_collection.to_s.pluralize.underscore).count
        remaining.times do
          @units << {
